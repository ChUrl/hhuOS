...
readonly CONST_QEMU_CPU_I386="base,+fpu,+tsc,+cmov,+fxsr,+mmx,+sse,+apic,+x2apic" # +x2apic not supported via TCG, needs KVM
...
readonly CONST_QEMU_ARGS="-boot d -vga std -serial stdio -rtc base=localtime -device isa-debug-exit -smp 2" # -smp 2 only works with +apic
