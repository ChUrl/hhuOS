...
void EthernetModule::readPacket(Util::Stream::ByteArrayInputStream &stream, LayerInformation information, Device::Network::NetworkDevice &device) {
    ...
    invokeNextLayerModule(ethernetHeader.getEtherType(), {ethernetHeader.getSourceAddress(), information.payloadLength - EthernetHeader::HEADER_LENGTH}, stream, device);
}
