...
IoApicInformation::IoApicInformation(const Acpi::IoApic *ioApic, const Acpi::NmiSource *nmiSource)
  : id(ioApic->ioApicId),
    ...
    nmiGsi(hasNmi ? static_cast<Kernel::GlobalSystemInterrupt>(nmiSource->globalSystemInterrupt) : Kernel::GlobalSystemInterrupt(0)),
    nmiPolarity(hasNmi ? (nmiSource->flags & Acpi::IntiFlag::ACTIVE_HIGH ? REDTBLEntry::PinPolarity::HIGH : REDTBLEntry::PinPolarity::LOW) : REDTBLEntry::PinPolarity::HIGH),
    nmiTriggerMode(hasNmi ? (nmiSource->flags & Acpi::IntiFlag::EDGE_TRIGGERED ? REDTBLEntry::TriggerMode::EDGE : REDTBLEntry::TriggerMode::LEVEL) : REDTBLEntry::TriggerMode::EDGE) {}
...
IoApicPlatform::IoApicIrqOverride::IoApicIrqOverride(const Acpi::InterruptSourceOverride *interruptSourceOverride)
  : bus(interruptSourceOverride->bus),
    ...
    polarity((interruptSourceOverride->flags & 0x3) == 0 ? REDTBLEntry::PinPolarity::BUS : (interruptSourceOverride->flags & Acpi::IntiFlag::ACTIVE_HIGH ? REDTBLEntry::PinPolarity::HIGH : REDTBLEntry::PinPolarity::LOW)),
    triggerMode((interruptSourceOverride->flags & 0xc) == 0 ? REDTBLEntry::TriggerMode::BUS : (interruptSourceOverride->flags & Acpi::IntiFlag::EDGE_TRIGGERED ? REDTBLEntry::TriggerMode::EDGE : REDTBLEntry::TriggerMode::LEVEL)) {}
...
IoApicPlatform::IoApicIrqOverride *IoApicPlatform::getIoApicIrqOverride(Kernel::GlobalSystemInterrupt target) const {
...
IoApicPlatform::IoApicIrqOverride *IoApicPlatform::getIoApicIrqOverride(InterruptRequest source) const {
    for (uint32_t i = 0; i < overrides.size(); ++i) {
        IoApicIrqOverride *override = overrides.get(i);
