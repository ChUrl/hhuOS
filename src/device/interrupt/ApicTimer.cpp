...
ApicTimer::ApicTimer() {
    ...
    LApic::writeLVT(LApic::TIMER, { .slot = Kernel::InterruptDispatcher::APICTIMER,
                                    .isMasked = true,
                                    .timerMode = LApic::LVT_Timer_Mode::PERIODIC });

    // TODO: Has to be calibrated (using PIT or RTC or probably just the TimeService)
    // NOTE: Setting this to values smaller than 0xFFFFFFFF doesn't work?
    // NOTE: Probably because the timer interrupt is bugged so if the timer counts down too fast it burns
...
void ApicTimer::plugin() {
    ...
    LApic::allow(LApic::TIMER); // TODO: This should be done in interruptService
}
